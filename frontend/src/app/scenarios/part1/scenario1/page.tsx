...
export default function Scenario1Page() {
  ...
  const [gender, setGender] = useState<'male' | 'female'>('male'); // Default or from selection
  ...
  const handleImageUpload = async (file: File) => {
    try {
      ...
      // const currentGender = gender; // Use the state variable for gender

      // Create form data for the image file only
      ...
      let pollCount = 0;
      const maxPolls = 60; // Poll for 2 minutes (60 * 2000ms)
      let pollingCompleted = false; // Flag to prevent multiple navigations/errors
      ...
      const pollInterval = setInterval(async () => {
        ...
        try {
          ...
          if (statusData.status_details && statusData.status_details.faceswap_status === 2) { // Success
            clearInterval(pollInterval);
            ...
            // Pass current scenario identifier
            router.push(`/generated-image?url=${encodeURIComponent(imageUrl)}&fromScenario=1`);
          } else if (statusData.status_details && statusData.status_details.faceswap_status === 3) { // Error
            clearInterval(pollInterval);
            pollingCompleted = true;
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-yellow-100 via-orange-100 to-red-100 p-4">
      <div className="w-full max-w-lg mx-auto bg-white/90 p-8 rounded-xl shadow-2xl text-center">
        <div className="flex justify-end mb-2">
          ...
          >
            &times; {/* Simple X icon for closing/going back */}
          </button>
        </div>
        ...
        <p className="text-gray-700 mb-6">
          이곳에서 직접 사진을 업로드하여 얼굴이 교체된 이미지를 만들어 보세요. 미나가 어떻게 변하는지 보여드릴게요!
        </p>
        ...
        {/* Gender selection UI - Placeholder, you can implement a proper one */}
        <div className="mb-6">
